...
title = My Clicker # замінити на назву застосунку
# (str) Package name
...
version = 0.1 # вказати версію застосунку
...
#icon.filename = %(source.dir)s/icon.png # шлях до іконки застосунку

# (list) Supported orientations
# Valid options are: landscape, portrait, portrait-reverse, landscape-reverse, or all
orientation = all # орієнтація екрану - portrait, landscape або all
...
#android.permissions = android.permission.INTERNET, (name=android.permission.WRITE_EXTERNAL_STORAGE;maxSdkVersion=18) дозволи на доступ застосунку до ресурсів
...
android.ndk_dir = /usr/local/lib/android/sdk/ndk-bundle # або закоментовано - по ситуації
# (str) Android SDK directory (if empty, it will be automatically downloaded.)
android.sdk_dir = /usr/local/lib/android/sdk # або закоментовано - по ситуації
...
android.skip_update = False # або закоментовано
...
#instead of PythonActivity
#android.activity_class_name = org.kivy.android.PythonActivity
# (str) Extra xml to write directly inside the <manifest> element of
#AndroidManifest.xml
# use that parameter to provide a filename from where to load your custom XML code
#android.extra_manifest_xml = ./src/android/extra_manifest.xml
# (str) Extra xml to write directly inside the <manifest><application> tag of
#AndroidManifest.xml
# use that parameter to provide a filename from where to load your custom XML
arguments:
#android.extra_manifest_application_arguments =
./src/android/extra_manifest_application_arguments.xml
# (str) Full name including package path of the Java class that implements Python
#Service
...
# (list) Gradle repositories to add {can be necessary for some
#android.gradle_dependencies}
...
# see https://developer.android.com/reference/tools/gradle-
#api/7.1/com/android/build/api/dsl/PackagingOptions
...
# (list) Copy these files to src/main/res/xml/ (used for example with intent-
#filters)
...
# (list) Android shared libraries which will be added to AndroidManifest.xml using
#<uses-library> tag
...
#x86_64
# In past, was `android.arch` as we weren't supporting builds for multiple archs at
#the same time.
android.archs = arm64-v8a, armeabi-v7a
# (int) overrides automatic versionCode computation (used in build.gradle)
# this is not the same as app version and should only be edited if you know what
#you're doing
...
# Usage example : android.manifest_placeholders =
#[myCustomUrl:\"org.kivy.customurl\"]
...
# (str) A display cutout is an area on some devices that extends into the display
#surface.
# It allows for an edge-to-edge experience while providing space for important
#sensors on the front of the device.

# Available options for Android API >= 28 are "default, shortEdges, never" and
#defaults to never.
...
#p4a.source_dir =
# (str) The directory in which python-for-android should look for your own build
#recipes (if any)
#p4a.local_recipes =
...
# "in the future" --use-setup-py is going to be the default behaviour in p4a, right
#now it is not
# Setting this to false will pass --ignore-setup-py, true will pass --use-setup-py
# NOTE: this is general setuptools integration, having pyproject.toml is enough, no
#need to generate
# setup.py if you're using Poetry, but you need to add "toml" to
#source.include_exts.
#p4a.setup_py = false
...
#ios.media_usage_description = "<APP> needs to access your media in order to <Do X
#and Y and Z> "

# (str) Justification text to be provided for being able to use local network
#ios.local_network_usage_description = "<App> needs permissions to <Do X and Y and
#Z> in your Local Area Network"
...
#ios.app_extensions = [["7zip", "zip"], ["public.zip-archive"],
#"org.kivy.myappextensionfile", "<MyCustom> Extension File",
#"${MACOSX_BUNDLE_ICON_FILE}", "http://mysite.com/myapp/extensions.html"],
# (str) URL pointing to .ipa file to be installed
# This option should be defined along with `display_image_url` and
#`full_size_image_url` options.
